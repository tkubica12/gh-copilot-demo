...
This is a sanitized example from a real local run with the Python runner, Copilot CLI backend, OpenTelemetry file export, and one measured iteration per variant. Treat the numbers as repeatable lab evidence, not universal pricing or billing data. Weighted units use the configurable rates in `model-pricing.toml`; GPT-5.5 and GPT-5.4 mini use the supplied Copilot price cards. They are not raw token sums: `input * input_rate + cache * cache_read_rate + output * output_rate + cache_create * cache_creation_rate`. For run-level details, see [reports/python-suite-2026-04-26.md](reports/python-suite-2026-04-26.md).

Separate tokenizer-only check: [language-token-benchmark.md](reports/language-token-benchmark.md) compares English and Czech prompt shapes so language-choice claims are measured, not assumed.

Compression is the easiest row to overread. This benchmark models fresh handoff sessions that keep loading new large context blocks while carrying a compact summary forward; it is closer to a fork/restart or durable handoff pattern than a claim that every `/compact` turn is cheaper. A real compaction must pay for summary output and may replace cheap cache-read history with full-price input. It only wins when the removed stale context is large enough, the summary improves quality, or the same distilled context is reused across multiple later sessions.

Model-routing and multi-agent rows should be read with the same caution. A smaller model or subagent is useful when the task is bounded and context is narrow, but rereading context that a stronger model already has cached can erase the saving. The goal is not cheapest token in isolation; it is the cheapest reliable completed task.
...
6. Compression is situational: it should be measured as a break-even between removed stale context, lost cache-read discount, summary output cost, and reuse count.
