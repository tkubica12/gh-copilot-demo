...
if echo "$TOOL_ARGS" | grep -qiE '(^|[[:space:]])rm[[:space:]]+-rf([[:space:]]|$)|Remove-Item.*-Recurse.*-Force|(^|[[:space:]])mkfs([[:space:]]|$)|(^|[[:space:]])format(\.com)?([[:space:]]|$)|Format-Volume|DROP[[:space:]]+TABLE|git[[:space:]]+push[[:space:]].*(--force(-with-lease)?|-f)($|[[:space:]])|curl.+\|.+bash|wget.+\|.+sh'; then
  jq -n \
    --arg decision "deny" \
