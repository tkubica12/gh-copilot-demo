...
async def lifespan(app: FastAPI):
    global credential, storage_account_client, container_client, servicebus_client
...
app = FastAPI(title="AI processing", description="API to process pictures", lifespan=lifespan)
...
async def process_image(file: UploadFile = File(...)):
    ...
    await container_client.upload_blob(name=blob_name, data=file.file, overwrite=False)

    # Send message to Service Bus
    message = ServiceBusMessage(json.dumps({"blob_name": blob_name, "id": guid}))
    async with servicebus_client.get_queue_sender(servicebus_queue_name) as sender:
